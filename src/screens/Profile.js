...
export default function Profile() {
    ...
    async function fetchUsersData() {
        ...
        if (loggedUserId) {
            const usersDbRef = firebase.app().database().ref(''); //add your databse name
            usersDbRef
                .child(loggedUserId)
                .once('value')
                .then(resp => {
        ...
        try {
            ...
            if (loggedUserId) {
                ...
                if (hasImageUploaded) {
                    ...
                    updateProfileInFirebase(_____,_____); // add here loggedin user id and their profile pic uri to this function
                }
            }
        } catch {
            setIsLoading(false);
    ...
    async function uploadImageInFirebase(loggedUserId) {
        ...
        return ___; // return response
    }

    //function to update user profile data in firebase db
    function updateProfileInFirebase(loggedUserId, imageUploadUrl) {
        const usersDbRef = firebase.app().database().ref(''); // add your database name
        usersDbRef
            .child() // add loggedinuserid here
            .update({
                name,
                ageGroup,
                    ...
                    <ScrollView style={styles.container}>
                        ...
                        <Text style={styles.label}>Performance</Text>
                        {
                            givenQuizCount ?
                                ...
                                : ____  //add  keyword to do nothing
                        }
                        <Text style={styles.totalData}>Total Questions attempted: {performanceData.total}</Text>

                        <View style={styles.divider}></View>
                        {/* Below, use BasicButton tag with text as 'Save' and onpressing  the button must trigger handleSaveBtnClick function */}
                       _____
                        <View style={styles.divider}></View>
                    </ScrollView>
            }
