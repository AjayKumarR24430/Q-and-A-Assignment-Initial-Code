...
export default function UserHome({ navigation }) {
    ...
    async function fetchUsersQuizes() {
        ...
        if (loggedUserId) {
            const quizesDbRef = firebase.app().database().ref(''); // add your firebase database name here
            quizesDbRef
                .once('value')
                .then(resp => {
                    ...
                    if (quizes) {
                        ...
                        quizzes._____; //add the function. reversing the quizzes to get the lastest quiz on top
                        setQuiz(quizzes);
                    }
                    setIsLoading(false);
                })
    ...
    function handleQuizItemClick(index) {
        ...
        // navigate to 'GiveQuiz' page, taking quiz[index] as params
        _____
    }
        ...
        <>
            {
                isLoading ?
                    ...
                    <ScrollView style={styles.container}>
                        {
                            // add suitable function to map the contents in the quiz variable
                            quiz.___((item, idx) => {
                                return (
                                    <QuizItem
                                        key={idx}
