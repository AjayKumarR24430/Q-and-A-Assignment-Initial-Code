...
export default function MyQuizes({ navigation }) {
    ...
    async function fetchUsersQuizes() {
        ...
        if (loggedUserId) {
            ...
            const quizesDbRef = firebase.app().database().ref(''); // add your firebase name
            quizesDbRef
            .on('value',
            function(resp){
                    ...
                    <ScrollView style={styles.container}>
                        {
                            myQuizzes.map((item, idx) => {
                                return (
                                    // add here
                                    // use the imported word 'QuizItem' functional tag here
                                    // and complete the map function
                                    ______
                                )
                            })
                        }
